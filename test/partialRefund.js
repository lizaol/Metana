...
describe("Testing partial refund contract", () => {
  ...
  it("should fail if msg.value is less than 1 ether", async function () {
    await expect(pr.minting({ value: "10" })).to.be.revertedWith("Not enough ether");
  });

  it("should mint 1000 tokens to the sender", async function () {
    await pr.minting({ value: "1000000000000000000" });
    expect(await pr.balanceOf(accounts[0].address)).to.equal(ethers.utils.parseUnits('1000', 18));
  });

  it("should fail if contract has no ether", async () =>{
    expect(await ethers.provider.getBalance(pr.address) == 0).to.be.revertedWith("Contract has no ether");
  })
  ...
  it("allows to withdraw", async () => {
    ...
    const balanceUserBefore = await ethers.provider.getBalance(accounts[0].address)
    tx = await pr.withdraw();
    await tx.wait()
    const balanceUserAfter = await ethers.provider.getBalance(accounts[0].address)
  ...
  it("should fail if amount > MAX_TOKENS", async () => {
    // console.log("SC eth balance", await ethers.provider.getBalance(pr.address))
    expect(pr.sellBack(1100000)).to.be.revertedWith("There not that many tokens");
  })
