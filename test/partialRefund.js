...
describe("Testing partial refund contract", () => {
  ...
  it("fails if total supply > MAX_TOKENS", async () => {
    console.log("max", await mock.MAX_TOKENS())
    console.log("pr max", await pr.MAX_TOKENS())
    console.log("pr totalSupply", await pr.totalSupply())
    console.log("mock totalSupply", await mock.totalSupply())
    await pr.minting({ value: "1000000000000000000" });
    await pr.minting({ value: "1000000000000000000" });
    expect(await pr.totalSupply() <= mock.MAX_TOKENS()).to.be.revertedWith("TotalSupply exceeds limit");

    // // expect(await pr.minting({ value: "1000000000000000000" })).to.be.revertedWith("TotalSupply exceeds limit");
    // console.log("pr balance", await pr.balanceOf(accounts[0].address))
    console.log("pr totalSupply", await pr.totalSupply())
    console.log("mock totalSupply", await mock.totalSupply())
  });
